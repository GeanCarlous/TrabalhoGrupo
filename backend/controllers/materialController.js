...
const uploadMaterial = async (req, res) => {
  try {
    ...
    if (!req.file) {
      return res
        .status(400)
        .json({ error: "O envio do ficheiro é obrigatório." });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Formato de data inválido. Use DD-MM-AAAA." });
    }
    ...
    if (
      isNaN(dataObtencaoDate.getTime()) ||
      dataObtencaoDate.getDate() !== parseInt(dia)
    ) {
      return res
        .status(400)
        .json({ error: "Data inválida. Verifique o dia e o mês." });
    }
    if (dataObtencaoDate > dataAtual) {
      return res
        .status(400)
        .json({ error: "A data de obtenção não pode ser uma data futura." });
    }
    ...
    if (!(await checkIfExists("disciplinas", disciplina))) {
      return res
        .status(400)
        .json({ error: `A disciplina '${disciplina}' não é válida.` });
    }
    if (!(await checkIfExists("tipos_material", tipoMaterial))) {
      return res
        .status(400)
        .json({ error: `O tipo de material '${tipoMaterial}' não é válido.` });
    }
    if (!(await checkIfExists("orientadores", orientador))) {
      return res
        .status(400)
        .json({ error: `O orientador '${orientador}' não é válido.` });
    }
...
const deleteMaterial = async (req, res) => {
  try {
    ...
    if (deletedRows === 0) {
      return res
        .status(403)
        .json({ error: "Você não tem permissão para apagar este material." });
    }
...
const findMaterials = async (req, res) => {
  try {
    ...
    const { search, disciplina, orientador } = req.query;

    const filters = {};
    if (search) filters.search = search;
    if (disciplina) filters.disciplina = disciplina;
    if (orientador) filters.orientador = orientador;
